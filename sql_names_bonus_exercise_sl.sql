...
-- (ANSER: There are 36 names with 15 letters. They all seem hyphenated.)
...
-- (ANSWER: There are 137 palindromes.)
...
-- (Hint: you might find this page helpful: https://www.postgresql.org/docs/current/functions-matching.html)
-- (ANSWER: 43 rows)
SELECT DISTINCT name
FROM names
WHERE name !~* '[aeiouy]';
...
-- (ANSWER: There 22,537 names with repeated letters.)
SELECT DISTINCT name
FROM names
WHERE name ~* '([a-z])\1'
ORDER BY name;
...
-- (ANSWER: There are 12 names with more than 2 repeated letters.)
SELECT DISTINCT name
FROM names
WHERE name ~* '([a-z])\1{2}'
ORDER BY name;
